...
void main() {
  ...
  group('TabPage', () {
    group('signed in', () {
      ...
      setUpAll(() {
        ...
        when(() => repository.myProfileHasLoaded).thenReturn(Completer()..complete());
      });
      ...
      testWidgets('Tapping Search goes to tab index 1', (tester) async {
        ...
        await tester
            .tap(find.ancestor(of: find.text('Search'), matching: find.byType(InkResponse)));
        expect(tester.state<TabPageState>(find.byWidget(tabPage)).currentIndex, 1);
      });
      testWidgets('Tapping Notifications goes to tab index 2 when signed in', (tester) async {
        final tabPage = TabPage();
        await tester.pumpApp(
          widget: MultiBlocProvider(
            providers: [
        ...
        await tester.runAsync(() => tester.tap(
            find.ancestor(of: find.text('Notifications'), matching: find.byType(InkResponse))));
        expect(tester.state<TabPageState>(find.byWidget(tabPage)).currentIndex, 2);
      });
      testWidgets('Tapping Profile goes to tab index 3 when signed in', (tester) async {
        final tabPage = TabPage();
        await tester.pumpApp(
          widget: MultiBlocProvider(
            providers: [
        ...
        await tester.runAsync(() async => await tester
            .tap(find.ancestor(of: find.text('Profile'), matching: find.byType(InkResponse))));
    ...
    group('not signed in', () {
      ...
      testWidgets('Tapping Notifications goes to LoginPage when not signed in', (tester) async {
        await tester.pumpApp(
          widget: MultiBlocProvider(
            providers: [
              BlocProvider<NotificationCubit>(
                  create: (context) =>
                      NotificationCubit(repository: RepositoryProvider.of<Repository>(context))),
            ],
            child: TabPage(),
          ),
          repository: repository,
      ...
      testWidgets('Tapping Profile goes to LoginPage when not signed in', (tester) async {
        await tester.pumpApp(
          widget: MultiBlocProvider(
            providers: [
              BlocProvider<NotificationCubit>(
      ...
      testWidgets('Tapping record button when not signed in will open LoginPage', (tester) async {
        await tester.pumpApp(
          widget: MultiBlocProvider(
            providers: [
              BlocProvider<NotificationCubit>(
        ...
        await tester.runAsync(() async => await tester.tap(find.byType(RecordButton)));
        await tester.pumpAndSettle();
        expect(find.byType(LoginPage), findsOneWidget);
      });
    });
    ...
    group('signed in, but no profile', () {
      ...
      setUpAll(() {
        ...
        when(() => repository.getProfileDetail('aaa')).thenAnswer((invocation) async => null);
        when(() => repository.hasAgreedToTermsOfService).thenAnswer((invocation) async => true);
      });
  ...
  group('NotificationDot', () {
    setUpAll(() {
      registerFallbackValue(DateTime.parse('2021-05-20T00:00:00.00'));
    });
    group('has notifications', () {
      late final Repository repository;
      setUpAll(() {
        repository = MockRepository();
        when(() => repository.statusKnown).thenReturn(Completer<void>()..complete());
        when(() => repository.myProfileHasLoaded).thenReturn(Completer<void>()..complete());
      ...
      testWidgets('Notification dots are shown properly', (tester) async {
        when(() => repository.notificationsStream).thenAnswer((invocation) => Stream.value([
              ...
              AppNotification(
                ...
                actionUid: 'aaa',
                actionUserName: 'Tyler',
                commentText: 'something random @b35bac1a-8d4b-4361-99cc-a1d274d1c4d2'
    ...
    group('does not have notifications', () {
      testWidgets('Users without notifications should not see any Notification Dot',
          (tester) async {
  ...
  group('Opening record page', () {
    ...
    testWidgets('Users with location permission will be able to go to record page', (tester) async {
      ...
      when(() => repository.mapVideosStream).thenAnswer((_) => Stream.value([]));
      when(() => repository.userId).thenReturn('aaa');
      when(() => repository.getProfileDetail('aaa')).thenAnswer((_) => Future.value(sampleProfile));
      when((() => repository.profileStream)).thenAnswer((_) => Stream.value({}));
      when(() => repository.getVideosFromUid('aaa')).thenAnswer((_) => Future.value([]));
